...
**Active tactical plan: [`plans/m10-orientation.md`](plans/m10-orientation.md)** — read it before starting M10 work; it holds the step order, the per-step Done-when, the container constraints inherited from M1 that the work must not break, and what is deliberately deferred to M6 and v2.
