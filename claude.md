...
`specs.md` holds the requirements (each with an ID, a version marker, and a verification Check) and the decision log. `plan-v1.md` breaks v1 into milestones M0–M9. **M0 is complete**: the app was moved from a single HTML file onto Vite + TypeScript with no behavior change, and the Mini bead pitch was corrected to 2.6 mm (spec Q1). **M1 — replacing the element-per-bead grid with canvas rendering (decision D1) — is in flight**: steps 1–5 of its tactical plan are done (canvas rendering, zoom by redraw, drag-pan plus the screen → cell mapping, bead codes with the toggle, and hiding codes below an 18 px legibility threshold), leaving step 6 (device pixel ratio, plus the performance check — which also re-checks the step 5 threshold, calibrated at 1x). The pipeline is still nearest-RGB, so M2's perceptual matching (D2), the color limit, crop, the correction editor, PNG export, and autosave are all still ahead.
